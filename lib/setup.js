 ...
 */
 ...
 function createGrower() {
    return getParticipantRegistry('org.example.model.Farmer')
        .then(function (participantRegistry) {
            var growerUser = getFactory().newResource('org.example.model', 'Farmer', 'GR_001');
            growerUser.companyName = "GrowerOne";
            var address = getFactory().newConcept('org.example.model', 'Address');
            address.country = 'Spain';
            growerUser.address = address;
            var growerUserT = getFactory().newResource('org.example.model', 'Farmer', 'GR_002');
